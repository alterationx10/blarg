...
object LinkValidator {
  ...
  /** Check if a link is internal and absolute (not external URL or relative
    * path) Only validates absolute internal links starting with "/" Relative
    * paths (like "../other.html") are skipped for validation
    */
  ...
  /** Normalize a link for comparison:
    ...
    *   - Add .html extension if missing and not a directory reference Note:
    *     This function expects absolute paths (starting with /)
    */
  def normalizeLink(href: String): String = {
    ...
    if normalized.endsWith("/") || normalized.endsWith(".html") then normalized
  ...
  /** Check if a normalized link exists in available pages Supports directory
    * indexes: /other or /other.html matches /other/index.html Returns (exists:
    * Boolean, attemptedPaths: List[String])
    */
  def linkExists(
      normalized: String,
    ...
    // Direct match
    attempts += normalized
    if availablePages.contains(normalized) then return (true, attempts.toList)
    ...
    if normalized.endsWith(".html") then {
      ...
      attempts += withIndex
      if availablePages.contains(withIndex) then return (true, attempts.toList)
    }
  ...
  /** Validate links in content against available pages Returns list of broken
    * links with source page info
    */
  def validateLinks(
  ...
  def reportBrokenLinks(brokenLinks: List[BrokenLink]): Unit = {
    if brokenLinks.nonEmpty then {
      ...
      brokenLinks.foreach { bl =>
        ...
        val pathsText =
          if bl.attemptedPaths.size == 1 then bl.attemptedPaths.head
          else bl.attemptedPaths.mkString(" and ")
        System.err.println(
          s"    Link to '${bl.targetHref}' (tried: $pathsText) - page not found"
        )
      }
      System.err.println()
    }
